...
class Ticket:
    ...
    def __str__(self):
        header = f"{Fore.WHITE}{Style.BRIGHT}{'TICKET DETAILS':^80}{Style.RESET_ALL}"
        details = f"""
{header}
#: {self.ticket_number}
-----
...
"""
    ...
    def print_requests(list_of_tickets):
        if not list_of_tickets:
            ...
            for ticket in list_of_tickets:
                ...
                print(
                    f"Solution: {'Waiting for resolution' if not ticket.solution else 'Answered... open request view more'}"
                )
    ...
    def print_ticket_details(self, ticket):
        ...
        print(ticket)
        print(
            f"{Fore.GREEN}|{' SOLUTION: ' + ('WAITING FOR RESOLUTION' if not ticket.solution else ticket.solution) + ' ' *  20}|{Style.RESET_ALL}\n"
        )
        print("-" * 30)
    ...
    def filter_phone(tickets):
        ...
        filtered_tickets = [
            ticket for ticket in tickets if ticket.client_phone == filter_value
        ]
        return filtered_tickets
    ...
    def filter_executor(tickets):
        ...
        while True:
            if filter_value == 1:
                ...
                print(
                    "CHOOSE OPTION: 0. BACK 1. SERVICE_DEPT 2. LOGISTICS_DEPT 3. SALES_DEPT"
                )
        return filtered_tickets
    ...
    def process_the_lead(ticket):
        print(
            "PROCESS THE LEAD AND PROVIDE A SOLUTION FOR THE TICKET. ONCE THE SOLUTION IS PROVIDED, THE TICKET WILL BE RETURNED TO THE OPERATOR FOR REVIEW"
        )
        while True:
            ...
            if len(solution) >= 10:
                ...
                print(
                    "SOLUTION ADDED SUCCESSFULLY. TICKET STATUS UPDATED TO 'RESOLVED'"
                )
                break
            else:
