...
class Operator(Ticket):
    ...
    def validate_name(self, value):
        if len(value) >= 3 and value.isalpha():
            ...
            raise ValueError(
                "INVALID CLIENT NAME. MUST BE AT LEAST 3 CHARACTERS AND CONTAIN ONLY LETTERS"
            )

    def validate_phone(self, value):
        phone_regex = r"^\+?\d{1,3}[-.\s]?\(?\d{1,4}\)?[-.\s]?\d{5,14}$"
    ...
    def collect_ticket_data(self):
        ...
        fields = [
            ...
            (
                "request_type",
                "Enter the request type (0: SERVICE,  1: PURCHASE,  2: LOGISTICS): ",
                self.validate_request_type,
            ),
            ("source", "Enter the source (0: CHAT,  1: CALL): ", self.validate_source),
            (
                "executor",
                "Enter the executor (0: SERVICE_DEPT,  1: LOGISTICS_DEPT,  2: SALES_DEPT): ",
    ...
    def redirect_request(ticket):
        while True:
            new_executor_number = int(
                input(
                    "SELECT THE NEW EXECUTOR: \n1. SERVICE_DEPT\n 2. LOGISTICS_DEPT\n 3. SALES_DEPT\n"
                ).strip()
            )
            ...
            if ticket.executor != new_executor:
                ...
                print(
                    f"NEW EXECUTOR FOR TICKET {ticket.ticket_number} SET SUCCESSFULLY: {new_executor}"
                )
            else:
    ...
    def change_request_status(ticket):
        while True:
            ...
            if ticket.status != new_status:
                ...
                print(
                    f"NEW STATUS FOR TICKET {ticket.ticket_number} SET SUCCESSFULLY: {new_status}"
                )
            else:
    ...
    def send_reminder(self, ticket):
        ...
        sender_email_data = os.getenv("sender_email_data")
        sender_password_data = os.getenv("sender_password_data")

        if (
            executor_email is None
            or sender_email_data is None
            or sender_password_data is None
        ):
            print(
                "One or more environment variables are not set. Please check your configuration."
            )
            return
    ...
    def print_operator_menu():
        print("OPERATOR MODE. PLEASE SELECT FUNCTION:")
